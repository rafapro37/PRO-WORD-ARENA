  ...
  return (
    <div className={`h-screen bg-[var(--bg-main)] border-r border-[var(--border)] flex flex-col transition-all duration-300 ${isRetracted ? 'w-20' : 'w-64'}`}>
      ...
      <nav className="flex-1 p-4 space-y-2">
        ...
        {/* MARKET - Available to most users */}
        {showMarket && (role === UserRole.ORGANIZER || role === UserRole.PLAYER || role === UserRole.TEAM_MANAGER || role === UserRole.GUEST) && (
             <div 
                onClick={() => onNavigate('find-players')} 
                className="flex items-center gap-3 p-3 rounded-lg cursor-pointer transition-colors border-l-4 text-[var(--text-secondary)] hover:bg-[var(--bg-card)] hover:text-[var(--text-main)]"
                style={currentPage === 'find-players' ? activeStyle : inactiveStyle}
             >
               <ShoppingBag size={20} />
               {!isRetracted && <span>Mercado da Bola</span>}
             </div>
        )}
